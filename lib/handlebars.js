...
handlebars.registerHelper('required_properties', (model, options)=>{
    let schemaName = getModelName(model);
    let schema = _swagger.definitions[schemaName];
...
//handlebars.registerHelper('show_examples', ()=>{
//   return _showExamples === true;
//} );
...
handlebars.registerHelper('get_model', (parentModel, property)=>{
    ...
    let currentModelName = parentModel.modelname;
...
handlebars.registerHelper('nested_model', (parentModel, property)=>{
    ...
    if(modelname !== currentModelName && modelname != null){
        ...
        let model = _swagger.definitions[modelname];

        for(let x=0; x< Object.keys(model.properties).length; x++){
            let propKey = Object.keys(model.properties)[x];

            let subModelname = getModelName(model.properties[propKey]);
